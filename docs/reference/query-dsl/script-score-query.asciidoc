...
functions] in the painless script. Besides these functions, there are a number
of predefined functions that can help you with scoring:
...
These functions take advantage of efficiencies from internal mechanisms.
...
===== Faster alternatives
The `script_score` query calculates the score for every hit, or matching
document. There are faster alternative query types that can efficiently skip
non-competitive hits:

* If you want to boost documents on some static fields, use the 
 <<query-dsl-rank-feature-query, `rank_feature`>> query.
 * If you want to boost documents closer to a date or geographic point, use
...
We are deprecating the <<query-dsl-function-score-query, `function_score`>> query. We recommend using the `script_score` query instead.
